...
var logoFrag = glslify( './assets/logo.fs' )
var logoVert = glslify( './assets/logo.vs' )
...
image.onload = _ => {
    ...
    var minRadius = 100, maxRadius = 200, radius = 0
    while( l-- >= 0 ){
    ...
    // document.onclick = _ => {
        TweenLite.to( logo.material, 2, { opacity: 0.95, delay: 8 })

        // TweenLite.to( material.uniforms.uOpacity, 2, { value: 0.2, delay: 16 })
    TweenLite.to( camera.position, 6, { z: 500 })
    // TweenLite.to( prop, 10, { converge: 1, delay:7 })
